...
DIFFICULTY_GUIDE = {
    ...
    "상": (
        ...
        "③ 후보 단어가 여러 개 떠오르면, 머릿속으로 '내가 이 단어를 내면 상대가 무엇으로 받을 수 있고, "
        "그다음엔 내가 또 무엇으로 받을 수 있을지'를 2~3수 정도 시뮬레이션해보고 가장 유리한 쪽을 선택해. "
        "④ 이건 '되도록'이 아니라 '예외 없는 필수 조건'이야: 네가 내는 모든 단어는 매번 반드시 "
        "'다음 사람이 그 글자로 시작하는 단어를 3개 이하로만 알 정도로 희귀한 음절'로 끝나야 해. "
        "다음 패턴에 해당하는 음절을 최우선으로 노려: "
        "화학 원소명 등 외래어 계열 어미(-늄, -륨, -튬, -퓸), 외래어/전문용어의 어미(-츠, -톤, -폰, -크), "
        "겹받침(ㄶ/ㄺ/ㄼ/ㅄ)이나 흔치 않은 겹모음(ㅢ/ㅘ/ㅝ/ㅟ)으로 끝나는 단어, "
        "그리고 두음법칙을 적용해도 시작 가능한 단어가 사실상 없는 음절. "
        "이런 음절로 끝나는 단어를 낼 수 있는데도 그냥 평범하고 무난한 단어를 내는 것은 "
        "실패한 수로 간주해. 매 턴 스스로에게 '이 단어가 정말 상대를 최대한 궁지에 몰고 있나?'를 "
        "물어보고, 아니라면 더 어려운 단어가 없는지 다시 생각해봐. "
