...
DIFFICULTY_GUIDE = {
    ...
    "상": (
        ...
        "진짜 고수는 단어 하나하나를 즉흥적으로 내지 않고, 몇 수 앞을 내다보고 판단해. "
        "너도 그렇게 해야 해: "
        "① 지금 낼 단어의 마지막 글자로 시작하는 단어가 상대에게 얼마나 많이 남아있는지 가늠해보고, "
        "그 선택지가 적을수록(특히 상대가 그 글자로 다시 너를 공격하기 어려울수록) 좋은 수야. "
        "② 반대로 '이', '기', '리', '개', '자' 등 워낙 다양한 단어로 이어질 수 있는 '안전 음절'로 "
        "끝나는 단어는 상대에게 선택의 폭만 넓혀주는 나쁜 수이니 피해. "
        "③ 매 턴마다 답을 하나만 떠올리고 끝내지 말고, 머릿속으로 최소 5~10개의 후보 단어를 "
        "먼저 쭉 브레인스토밍한 다음, 그중에서 고르는 순서로 생각해. "
        "④ 절대 규칙(예외 없음): 브레인스토밍한 후보들 중에 다음 '한방단어' 패턴에 해당하는 단어가 "
        "단 하나라도 있다면, 다른 어떤 이유(예: 너무 흔한 단어를 쓰고 싶다거나, 다양성을 주고 싶다거나)로도 "
        "무시하지 말고 반드시 그 단어를 내야 해. 한방단어를 쓸 수 있는데도 안 쓰는 것은 실패한 수야. "
        "한방단어 패턴: 화학 원소명 등 외래어 계열 어미(-늄, -륨, -튬, -퓸), 외래어/전문용어의 어미 "
        "(-츠, -톤, -폰, -크), 겹받침(ㄶ/ㄺ/ㄼ/ㅄ)이나 흔치 않은 겹모음(ㅢ/ㅘ/ㅝ/ㅟ)으로 끝나는 단어, "
        "그리고 두음법칙을 적용해도 시작 가능한 단어가 사실상 없는 음절. "
        "한방단어 후보가 하나도 없을 때만, 차선책으로 '다음 사람이 그 글자로 시작하는 단어를 "
        "3개 이하로만 알 정도로 희귀한 음절'로 끝나는 단어를 선택해. "
        "매 턴 스스로에게 '한방단어를 쓸 수 있었는데 놓친 건 아닌가?'를 반드시 재확인해. "
        "반드시 실제 존재하는 단어여야 하고, 3~6글자 정도의 단어를 적극적으로 사용해도 좋아. "
        "반대로 상대가 너에게 이런 어려운 음절을 던졌을 때는 절대 곧바로 포기하지 말고, "
        "다음 순서로 최대한 뒤져봐: "
...
if st.session_state.stage == "setup":
    st.caption("실시간 끝말잇기 대결! 시간 초과되면 바로 패배해요.")
